...
class BertAdam(Optimizer):
    ...
    def get_lr(self):
        ...
        for group in self.param_groups:
            for p in group['params']:
                state = self.state[p]
                # print('state[key]: {}'.format(state['step']))
                if len(state) == 0:
                    return [0]
                if group['t_total'] != -1:
    ...
    def reset_optimizer_states(self):
        for group in self.param_groups:
            for p in group['params']:
                # Exponential moving average of gradient values
                self.state[p]['next_m'] = torch.zeros_like(p.data)
                # Exponential moving average of squared gradient values
                self.state[p]['next_v'] = torch.zeros_like(p.data)
    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            for p in group['params']:
                ...
                # print('state[\'step\']: {}'.format(state['step']))
                ...
                # logger.info('lr_scheduled: {}'.format(lr_scheduled))
