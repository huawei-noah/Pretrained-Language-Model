...
class BertEmbeddings(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.word_embeddings = QuantizeEmbedding(config.vocab_size, config.hidden_size, padding_idx=0,
                                    clip_val=config.clip_init_val, weight_bits=config.weight_bits, input_bits=config.input_bits,
...
class BertSelfAttention(nn.Module):
    def __init__(self, config):
        ...
        # self.weight_quant_method = config.weight_quant_method
        # self.learnable_scaling = config.learnable_scaling

        # automatically skip quant if num_bits == 32.
...
class BertPreTrainedModel(nn.Module):
    ...
    def from_pretrained(cls, pretrained_model_name_or_path, *inputs, **kwargs):
        ...
        # config.__dict__['quantize_activation'] = kwargs.get('quantize_activation', 0)
        logger.info("Model config {}".format(config))
        # Instantiate model.
...
class TinyBertForSequenceClassification(BertPreTrainedModel):
    ...
    #     weight_quant_method=args.weight_quant_method, input_quant_method=args.input_quant_method,
    #     learnable_scaling=args.learnable_scaling, ACT2FN=args.ACT2FN,
    #     sym_quant=args.sym_quant, asym_rest=args.asym_rest)

class BertForSequenceClassification(BertPreTrainedModel):
    def __init__(self, config, num_labels, weight_bits=32, input_bits=32,
                 weight_quant_method='twn', input_quant_method='uniform',
                 clip_init_val=2.5, learnable_scaling=False, ACT2FN=None,
                 sym_quant_qkvo=True, sym_quant_ffn_attn=False,
                 embed_layerwise=False, weight_layerwise=True, input_layerwise=True):
        super(BertForSequenceClassification, self).__init__(config)

        config.weight_bits = weight_bits
        config.input_bits = input_bits
        config.weight_quant_method = weight_quant_method
        config.input_quant_method = input_quant_method
        config.clip_init_val = clip_init_val
        config.learnable_scaling = learnable_scaling
        config.sym_quant_qkvo = sym_quant_qkvo
        config.sym_quant_ffn_attn = sym_quant_ffn_attn
        config.embed_layerwise = embed_layerwise
        config.weight_layerwise = weight_layerwise
        config.input_layerwise = input_layerwise

        if ACT2FN:
            config.hidden_act = ACT2FN
