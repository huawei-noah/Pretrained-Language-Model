# coding=utf-8
# 2021.07.16 - Add tws quantization modules and some changes on the modeling
...
from .utils_quant import QuantizeLinear, QuantizeEmbedding, act_quant_fn, LsqStepSize
...
class BertEmbeddings(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.word_embeddings_1 = QuantizeEmbedding(config.vocab_size, config.hidden_size, padding_idx=0,
        ...
        self.word_embeddings_2 = QuantizeEmbedding(config.vocab_size, config.hidden_size, padding_idx=0,
                                                 clip_val=config.clip_init_val,
                                                 weight_bits=config.weight_bits,
                                                 input_bits=config.input_bits,
                                                 weight_quant_method=config.weight_quant_method,
    ...
    def forward(self, input_ids, token_type_ids=None):
        ...
        words_embeddings = self.word_embeddings_1(input_ids) + self.word_embeddings_2(input_ids)
...
class BertSelfAttention(nn.Module):
    def __init__(self, config):
        ...
        if config.dyna_hidden_size % config.num_attention_heads != 0:
            raise ValueError(
                "The hidden size (%d) is not a multiple of the number of attention "
                "heads (%d)" % (config.hidden_size, config.num_attention_heads))
        self.num_attention_heads = config.num_attention_heads
        self.attention_head_size = int(
            config.dyna_hidden_size / config.num_attention_heads)
        ...
        self.query_1 = QuantizeLinear(config.hidden_size, self.all_head_size, clip_val=config.clip_init_val,
        ...
        self.query_2 = QuantizeLinear(config.hidden_size, self.all_head_size, clip_val=config.clip_init_val,
                                    weight_bits=config.weight_bits, input_bits=config.input_bits,
                                    weight_layerwise=config.weight_layerwise, input_layerwise=config.input_layerwise,
                                    weight_quant_method=config.weight_quant_method,
                                    input_quant_method=config.input_quant_method,
        ...
        self.key_1 = QuantizeLinear(config.hidden_size, self.all_head_size, clip_val=config.clip_init_val,
                                  weight_bits=config.weight_bits, input_bits=config.input_bits,
                                  weight_layerwise=config.weight_layerwise, input_layerwise=config.input_layerwise,
                                  weight_quant_method=config.weight_quant_method,
                                  input_quant_method=config.input_quant_method,
                                  learnable=config.learnable_scaling, symmetric=config.sym_quant_qkvo)
        self.key_2 = QuantizeLinear(config.hidden_size, self.all_head_size, clip_val=config.clip_init_val,
                                  weight_bits=config.weight_bits, input_bits=config.input_bits,
                                  weight_layerwise=config.weight_layerwise, input_layerwise=config.input_layerwise,
                                  weight_quant_method=config.weight_quant_method,
                                  input_quant_method=config.input_quant_method,
        ...
        self.value_1 = QuantizeLinear(config.hidden_size, self.all_head_size, clip_val=config.clip_init_val,
                                    weight_bits=config.weight_bits, input_bits=config.input_bits,
                                    weight_layerwise=config.weight_layerwise, input_layerwise=config.input_layerwise,
                                    weight_quant_method=config.weight_quant_method,
                                    input_quant_method=config.input_quant_method,
                                    learnable=config.learnable_scaling, symmetric=config.sym_quant_qkvo)
        self.value_2 = QuantizeLinear(config.hidden_size, self.all_head_size, clip_val=config.clip_init_val,
                                    weight_bits=config.weight_bits, input_bits=config.input_bits,
                                    weight_layerwise=config.weight_layerwise, input_layerwise=config.input_layerwise,
                                    weight_quant_method=config.weight_quant_method,
                                    input_quant_method=config.input_quant_method,
        ...
        # TODO: check whether using the same clipping as before split
        ...
        elif config.input_quant_method == 'lsq' and config.input_bits < 32:
            self.clip_query = LsqStepSize(torch.tensor(1.0))  # stepsize will be initialized in the first quantization
            self.clip_key = LsqStepSize(torch.tensor(1.0))
            self.clip_value = LsqStepSize(torch.tensor(1.0))
            self.clip_attn = LsqStepSize(torch.tensor(1.0))
    ...
    def forward(self, hidden_states, attention_mask, output_att=False):
        # support binary split
        mixed_query_layer = self.query_1(hidden_states) + self.query_2(hidden_states)
        mixed_key_layer = self.key_1(hidden_states) + self.key_2(hidden_states)
        mixed_value_layer = self.value_1(hidden_states) + self.value_2(hidden_states)
...
class BertSelfOutput(nn.Module):
    def __init__(self, config):
        ...
        # support binary split
        self.dense_1 = QuantizeLinear(config.dyna_hidden_size, config.hidden_size, clip_val=config.clip_init_val,
                                    weight_bits=config.weight_bits, input_bits=config.input_bits,
                                    weight_layerwise=config.weight_layerwise, input_layerwise=config.input_layerwise,
                                    weight_quant_method=config.weight_quant_method,
                                    input_quant_method=config.input_quant_method,
                                    learnable=config.learnable_scaling, symmetric=config.sym_quant_qkvo)
        self.dense_2 = QuantizeLinear(config.dyna_hidden_size, config.hidden_size, clip_val=config.clip_init_val,
                                      weight_bits=config.weight_bits, input_bits=config.input_bits,
                                      weight_layerwise=config.weight_layerwise, input_layerwise=config.input_layerwise,
                                      weight_quant_method=config.weight_quant_method,
                                      input_quant_method=config.input_quant_method,
    ...
    def forward(self, hidden_states, input_tensor):
        # support binary split
        hidden_states = self.dense_1(hidden_states) + self.dense_2(hidden_states)
...
class BertIntermediate(nn.Module):
    def __init__(self, config):
        ...
        self.dense_1 = QuantizeLinear(config.hidden_size, config.intermediate_size, clip_val=config.clip_init_val,
                                    weight_bits=config.weight_bits, input_bits=config.input_bits,
                                    weight_layerwise=config.weight_layerwise, input_layerwise=config.input_layerwise,
                                    weight_quant_method=config.weight_quant_method,
                                    input_quant_method=config.input_quant_method,
                                    learnable=config.learnable_scaling, symmetric=config.sym_quant_qkvo)
        self.dense_2 = QuantizeLinear(config.hidden_size, config.intermediate_size, clip_val=config.clip_init_val,
                                    weight_bits=config.weight_bits, input_bits=config.input_bits,
                                    weight_layerwise=config.weight_layerwise, input_layerwise=config.input_layerwise,
                                    weight_quant_method=config.weight_quant_method,
                                    input_quant_method=config.input_quant_method,
...
class BertOutput(nn.Module):
    def __init__(self, config):
        ...
        self.dense_1 = QuantizeLinear(config.intermediate_size, config.hidden_size, clip_val=config.clip_init_val,
                                    weight_bits=config.weight_bits, input_bits=config.input_bits,
                                    weight_layerwise=config.weight_layerwise, input_layerwise=config.input_layerwise,
                                    weight_quant_method=config.weight_quant_method,
                                    input_quant_method=config.input_quant_method,
                                    learnable=config.learnable_scaling, symmetric=config.sym_quant_ffn_attn)
        self.dense_2 = QuantizeLinear(config.intermediate_size, config.hidden_size, clip_val=config.clip_init_val,
                                      weight_bits=config.weight_bits, input_bits=config.input_bits,
                                      weight_layerwise=config.weight_layerwise, input_layerwise=config.input_layerwise,
                                      weight_quant_method=config.weight_quant_method,
                                      input_quant_method=config.input_quant_method,
...
class BertPooler(nn.Module):
    def __init__(self, config, recurs=None):
        ...
        self.dense_1 = QuantizeLinear(config.hidden_size, config.hidden_size, clip_val=config.clip_init_val,
                                    weight_bits=config.weight_bits, input_bits=config.input_bits,
                                    weight_layerwise=config.weight_layerwise, input_layerwise=config.input_layerwise,
                                    weight_quant_method=config.weight_quant_method,
                                    input_quant_method=config.input_quant_method,
                                    learnable=config.learnable_scaling, symmetric=config.sym_quant_qkvo)
        self.dense_2 = QuantizeLinear(config.hidden_size, config.hidden_size, clip_val=config.clip_init_val,
                                      weight_bits=config.weight_bits, input_bits=config.input_bits,
                                      weight_layerwise=config.weight_layerwise, input_layerwise=config.input_layerwise,
                                      weight_quant_method=config.weight_quant_method,
                                      input_quant_method=config.input_quant_method,
    ...
    def forward(self, hidden_states):
        ...
        pooled_output = self.dense_1(pooled_output) + self.dense_2(pooled_output)
...
class BertForSequenceClassification(BertPreTrainedModel):
    ...
    def forward(self, input_ids, token_type_ids=None,
                ...
                output_att=False, output_hidden=False, is_student=False):
