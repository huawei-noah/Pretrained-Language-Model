...
class SequenceSummary(nn.Module):
    r""" Compute a single vector summary of a sequence hidden states according to various possibilities:
        Args of the config class:
            ...
            summary_proj_to_labels: If True, the projection output to config.num_labels classes (otherwise to hidden_size). Default: False.
...
def make_divisible(input_size, width_mult, num_heads, min_value=1):
...
class USLinear(nn.Linear):
    def __init__(self, in_features, out_features, num_heads, bias=True, us=[True, True]):
    ...
    def forward(self, input):
        if self.us[0]:
            self.in_features = make_divisible(self.in_features_max, self.width_mult, self.num_heads)
        if self.us[1]:
            self.out_features = make_divisible(self.out_features_max, self.width_mult, self.num_heads)
