  ...
  #      super().__init__(args)
   #     self._optimizer = Adafactor(params, **self.optimizer_config)
    ...
    #def add_args(parser):
        ...
        #parser.add_argument('--beta2', type=float, default=-0.8, metavar="D",
        ...
        for group in self.param_groups:
            for p in group["params"]:
                ...
                if factored:
                    ...
                    exp_avg_sq_row.mul_(group["beta2"]).add_(
                        update.mean(dim=-1), alpha=1.0 - group["beta2"]
                    )
                    exp_avg_sq_col.mul_(group["beta2"]).add_(
                        update.mean(dim=-2), alpha=1.0 - group["beta2"]
                    )
                    ...
                    exp_avg_sq.mul_(group["beta2"]).add_(update, alpha=1.0 - group["beta2"])
                ...
                state['adafa_step'] = torch.norm(update)
