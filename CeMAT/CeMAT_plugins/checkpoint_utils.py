...
def load_model_ensemble_and_task(
    ...
    for filename in filenames:
        ...
        for shard_idx in range(num_shards):
            ...
            model.load_state_dict(state["model"], strict=strict, model_cfg=cfg.model, arg_task=arg_task)
...
def prune_state_dict(state_dict, model_cfg: Optional[DictConfig], arg_task: Optional[Namespace]):
    ...
    # if src is exo lang,we can't init the embedding with pre-training params because arch don't mach.
    # if tgt is exo lang,we can't init the embedding and out_prejection params because arch do't mach.
    # logger.info("arg_task:{}".format(arg_task))
    init_embedding_src = getattr(arg_task, "src_exo_lang", False)
    init_embedding_tgt = getattr(arg_task, "tgt_exo_lang", False)
    is_infer = getattr(arg_task, "is_exo_infer", False)
    # logger.info("is_infer:{}".format(is_infer))
    # logger.info("init_embedding_src:{}".format(init_embedding_src))
    # logger.info("init_embedding_tgt:{}".format(init_embedding_tgt))

    if init_embedding_src and "encoder.embed_tokens.weight" in state_dict and not is_infer:
        logger.info("encoder.embed_tokens.weight will not init by pre-training model.")
        del state_dict["encoder.embed_tokens.weight"]
        if "encoder.encoder_output_projection.weight" in state_dict:
            logger.info("encoder.encoder_output_projection.weight will not init by pre-training model.")
            del state_dict["encoder.encoder_output_projection.weight"]
    if init_embedding_tgt and "decoder.embed_tokens.weight" in state_dict and not is_infer:
        logger.info("decoder.embed_tokens.weight and  output_projection.weight will not init by pre-training model.")
        del state_dict['decoder.embed_tokens.weight']
        del state_dict["decoder.output_projection.weight"]

    plus_encoder_loss = getattr(model_cfg, "plus_encoder_loss", False)
    logger.info("plus_encoder_loss:{}".format(plus_encoder_loss))
    if not plus_encoder_loss and "encoder.encoder_output_projection.weight" in state_dict:
        del state_dict["encoder.encoder_output_projection.weight"]
        if "decoder.output_projection.bias" in state_dict:
            del state_dict["decoder.output_projection.bias"]
        if "encoder.encoder_output_projection.bias" in state_dict:
            del state_dict["encoder.encoder_output_projection.bias"]
    else:
