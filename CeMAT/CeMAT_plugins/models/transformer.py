...
from .fairseq_encoder import CematEncoder
...
@register_model("Bitransformer")
class BiTransformerModel(FairseqEncoderDecoderModel):
    """
    ...
    def freeze_module_params(cls, m):
        if m is not None:
            for p in m.parameters():
                p.requires_grad = False
                # print("{} has been freeze".format(p))

    def __init__(self, args, encoder, decoder):
        super().__init__(encoder, decoder)
        self.args = args
        self.supports_align_args = True
        if hasattr(args, "freeze_encoder_layer") and args.freeze_encoder_layer:
            for layer in range(args.encoder_layers):
                # print("{} has been freeze".format(encoder.layers[layer]))
                self.freeze_module_params(encoder.layers[layer])
        if hasattr(args, "freeze_encoder_emb") and args.freeze_encoder_emb:
            self.freeze_module_params(encoder.embed_tokens)
        if hasattr(args, "freeze_cross_attention") and args.freeze_cross_attention:
            for layer in range(args.decoder_layers):
                self.freeze_module_params(decoder.layers[layer].encoder_attn)
    ...
    def add_args(parser):
        ...
        parser.add_argument('--mask_self_att', default=False, action='store_true',
                            help='using mask self attention for decoders')
        ...
        parser.add_argument('--bi_self_att', default=False, action='store_true',
                            help='bi_self_att for decoder.')
        # fmt: on
    ...
    def forward(
        ...
        if hasattr(self.args, "plus_encoder_loss") and self.args.plus_encoder_loss:
            return encoder_out["mlm_out"], decoder_out
        else:
            return decoder_out
    ...
    @torch.jit.export
    def get_normalized_probs_may_encoder(
            self,
            net_output: Tuple[Tensor, Optional[Dict[str, List[Optional[Tensor]]]]],
            is_encoder: bool,
            log_probs: bool,
        ...
        """Get normalized probabilities (or log probs) from a net's output."""
        return self.get_normalized_probs_scriptable_may_encoder(net_output, is_encoder, log_probs, sample)


class TransformerEncoder(CematEncoder):
    """
    ...
    def __init__(self, args, dictionary, embed_tokens):
        ...
        self.output_embed_dim = args.encoder_output_dim
        ...
        self.args = args
        if hasattr(args, "plus_encoder_loss") and args.plus_encoder_loss:
            self.project_out_dim = None
            self.encoder_output_projection = nn.Linear(
                self.output_embed_dim, len(dictionary), bias=False
            )
            nn.init.normal_(
                self.encoder_output_projection.weight, mean=0, std=self.output_embed_dim ** -0.5
            )
    ...
    def forward(
        ...
        if hasattr(self.args, "plus_encoder_loss") and self.args.plus_encoder_loss:
            # T x B x C -> B x T x C
            mlm_output = x.transpose(0, 1)
            if self.project_out_dim is not None:
                mlm_output = self.project_out_dim(mlm_output)
            mlm_output = self.encoder_output_projection(mlm_output)
        # The Pytorch Mobile lite interpreter does not supports returning NamedTuple in
        # `foward` so we use a dictionary instead.
        # TorchScript does not support mixed values so the values are all lists.
        # The empty list is equivalent to None.
        return {
            "mlm_out": [mlm_output],  # output for bert
...
class TransformerDecoder(FairseqIncrementalDecoder):
    ...
    def __init__(self, args, dictionary, embed_tokens, no_encoder_attn=False):
        ...
        self.bi_self_att = getattr(args, "bi_self_att", False)
        print("note:you are using decoder is mask self attention?==========", not self.bi_self_att)
    ...
    def extract_features_scriptable(
        ...
        if self.cross_self_attention or prev_output_tokens.eq(self.padding_idx).any():
            # print("!!!!!!!!!!!!!!use cross self attention!!!!!!!!!!!!!",self.cross_self_attention , prev_output_tokens.eq(self.padding_idx).any())
            self_attn_padding_mask = prev_output_tokens.eq(self.padding_idx)

        # decoder layers
        attn: Optional[Tensor] = None
        inner_states: List[Optional[Tensor]] = [x]
        # self.no_mask_att = True
        for idx, layer in enumerate(self.layers):
            if incremental_state is None and not full_context_alignment and not self.bi_self_att:
...
@register_model_architecture("Bitransformer", "Bitransformer")
...
@register_model_architecture("Bitransformer", "cemat_transformer_noah")
def transformer_noah(args):
...
@register_model_architecture("Bitransformer", "cemat_transformer_base")
...
@register_model_architecture("Bitransformer", "transformer_base")
def bart_base_architecture(args):
    args.encoder_embed_dim = getattr(args, "encoder_embed_dim", 512)
    args.encoder_ffn_embed_dim = getattr(args, "encoder_ffn_embed_dim", 4 * 512)
    args.encoder_attention_heads = getattr(args, "encoder_attention_heads", 8)
...
@register_model_architecture("Bitransformer", "cemat_transformer_big")
def bart_base_architecture(args):
...
@register_model_architecture("Bitransformer", "transformer_big")
def bart_base_architecture(args):
    args.encoder_embed_dim = getattr(args, "encoder_embed_dim", 1024)
    args.encoder_ffn_embed_dim = getattr(args, "encoder_ffn_embed_dim", 4 * 1024)
    args.encoder_attention_heads = getattr(args, "encoder_attention_heads", 16)
