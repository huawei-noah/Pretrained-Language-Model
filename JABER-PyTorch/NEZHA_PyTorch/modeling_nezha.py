...
class NeZhaForTokenClassification(BertPreTrainedModel):
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, positions=None, seq_len=None, labels=None):
        r'''
        Return:
        loss
...
# class ColBERT_v2(BertPreTrainedModel):
#     def __init__(self, config, mask_punctuation, dim=128):
#         super(ColBERT_v2, self).__init__(config)
#         self.dim = dim
#         self.mask_punctuation = mask_punctuation
#         self.skiplist = {}
...
#                 token_type_ids_1=None, token_type_ids_2=None, positions_pos=None,
#                 positions_neg=None, similarity_metric="cosine", D=None, labels=None):

#         Q = self.query(input_ids_1, attention_mask_1, token_type_ids_1)
#         if D is None:
#             D = self.doc(input_ids_2, attention_mask_2, token_type_ids_2)
#         score, similarity_matrix = self.score(Q,D,similarity_metric)
#         if positions_pos is not None and positions_neg is not None:
#             scores_pos = torch.index_select(score,0, positions_pos)
#             scores_neg = torch.index_select(score,0, positions_neg)
#             final_scores = torch.stack((scores_pos, scores_neg), dim=0)
#         else:
#             final_scores = score
#         outputs = (final_scores,similarity_matrix)
#         return outputs   # score, similarity_matrix
